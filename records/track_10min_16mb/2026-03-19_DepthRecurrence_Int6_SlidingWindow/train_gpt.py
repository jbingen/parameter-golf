"""
Submission: Depth Recurrence + Int6 QAT + Sliding Window Eval
- 9 unique transformer blocks × N cycles for depth recurrence
- Int6 per-row quantization with QAT (straight-through estimator)
- FP16 tied embedding passthrough
- Wider MLP (3x expansion)
- Sliding window evaluation for long-context BPB
- NTK-aware RoPE scaling for eval_seq_len > train_seq_len
- Tuned optimizer hyperparameters
"""
...
class Hyperparameters:
    ...
    # Depth recurrence
    num_cycles = int(os.environ.get("NUM_CYCLES", 2))
...
def compute_bpb_for_chunk(
    ...
    """Compute total bytes for a chunk of (prev, target) token pairs."""
...
def eval_val_sliding(
    ...
    """Sliding window evaluation using per-position losses. Uses base_model (uncompiled)."""
    eval_sl = args.eval_seq_len if args.eval_seq_len > 0 else args.train_seq_len
    stride = min(args.eval_stride, eval_sl)
    ...
    with torch.inference_mode():
        for win_start in my_starts:
            win_end = min(win_start + eval_sl, n_tokens)
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                per_tok = base_model.per_token_loss(x, y).detach()

            # First window scores all positions; subsequent windows score last `stride`
            if win_start == 0:
                score_start = 0
            else:
                score_start = seq_len - min(stride, seq_len)
            ...
            val_byte_count += compute_bpb_for_chunk(
                prev_ids, tgt_ids, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut
            )
...
def eval_val(
    ...
    """Standard (non-sliding) validation for periodic checks during training."""
...
INT_KEEP_FLOAT_FP32_NAME_PATTERNS = tuple(
    pattern
    for pattern in os.environ.get(
        "INT_KEEP_FLOAT_FP32_NAME_PATTERNS",
...
def keep_float_tensor(name: str, t: Tensor, passthrough_orig_dtypes: dict[str, str]) -> Tensor:
    if any(pattern in name for pattern in INT_KEEP_FLOAT_FP32_NAME_PATTERNS):
        return t.float().contiguous()
    if t.dtype in {torch.float32, torch.bfloat16}:
        passthrough_orig_dtypes[name] = str(t.dtype).removeprefix("torch.")
        return t.to(dtype=INT_KEEP_FLOAT_STORE_DTYPE).contiguous()
    return t


def quantize_float_tensor(t: Tensor, quant_bits: int = 6) -> tuple[Tensor, Tensor]:
...
def quantize_state_dict(state_dict: dict[str, Tensor], quant_bits: int = 6, fp16_embed: bool = True):
    ...
    stats = dict.fromkeys(
        ("param_count", "num_tensors", "num_float_tensors", "num_nonfloat_tensors", "baseline_tensor_bytes", "int_payload_bytes"),
        0,
    )
    ...
    for name, tensor in state_dict.items():
        ...
        if fp16_embed and any(pattern in name for pattern in FP16_PASSTHROUGH_NAME_PATTERNS):
        ...
        q, s = quantize_float_tensor(t, quant_bits)
        if s.ndim > 0:
...
class Rotary(nn.Module):
    ...
    def set_base(self, new_base: float) -> None:
        """Update RoPE frequency base (for NTK-aware eval scaling)."""
        self.base = new_base
...
class GPT(nn.Module):
    def __init__(
        ...
        # Per-cycle normalization for stable recurrence
        self.cycle_norms = nn.ModuleList([RMSNorm() for _ in range(max(num_cycles - 1, 0))])
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        h = self._encode(input_ids).reshape(-1, self.tok_emb.weight.size(1))
    ...
    def per_token_loss(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        """Returns unreduced per-token CE losses. Shape: (batch * seq_len,)."""
        h = self._encode(input_ids).reshape(-1, self.tok_emb.weight.size(1))
        logits = self._logits(h)
...
def set_qat(model: nn.Module, enabled: bool, bits: int = 6) -> None:
...
def set_rope_eval_base(model: nn.Module, train_seq_len: int, eval_seq_len: int, base: float) -> None:
    ...
    for module in model.modules():
        if isinstance(module, Rotary):
            ...
            module.set_base(new_base)


def restore_rope_base(model: nn.Module, base: float) -> None:
    for module in model.modules():
        if isinstance(module, Rotary):
...
def main() -> None:
    ...
    eval_sl = args.eval_seq_len if args.eval_seq_len > 0 else args.train_seq_len
    val_tokens = load_validation_tokens(args.val_files, min(args.train_seq_len, eval_sl))
    ...
    # cycle_norms parameters
    for name, p in base_model.cycle_norms.named_parameters():
    ...
    log0(f"num_cycles:{args.num_cycles} effective_depth:{args.num_layers * args.num_cycles}")
    log0(f"mlp_mult:{args.mlp_mult} quant_bits:{args.quant_bits} fp16_embed:{args.fp16_embed}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(f"eval_seq_len:{eval_sl} eval_stride:{args.eval_stride}")
    log0(
    ...
    quant_obj, quant_stats = quantize_state_dict(base_model.state_dict(), args.quant_bits, args.fp16_embed)
    ...
    if master_process:
        with open("final_model.int_quant.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = os.path.getsize("final_model.int_quant.ptz")
        code_bytes = len(code.encode("utf-8"))
    ...
    if distributed:
        dist.barrier()
    with open("final_model.int_quant.ptz", "rb") as f:
    ...
    # Sliding window eval on roundtripped model (uses base_model for per-token losses)
    if eval_sl > args.train_seq_len:
        set_rope_eval_base(base_model, args.train_seq_len, eval_sl, args.rope_base)
    torch.cuda.synchronize()
    ...
    torch.cuda.synchronize()
    log0(
        f"final_sliding_window val_loss:{slide_val_loss:.4f} val_bpb:{slide_val_bpb:.4f} "
    ...
    if eval_sl > args.train_seq_len:
        restore_rope_base(base_model, args.rope_base)
